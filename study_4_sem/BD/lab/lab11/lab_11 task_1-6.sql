...
DECLARE @id int, @name char(40);
	...
	PRINT 'первая строка: ' + cast(@id as char) + trim(@name);
		FETCH NEXT from nameStudentCursor INTO @id, @name
	PRINT 'следующая строка: ' + cast(@id as char) + trim(@name);
		FETCH LAST from nameStudentCursor INTO @id, @name
	PRINT 'последняя строка: ' + cast(@id as char) + trim(@name);
		FETCH PRIOR from nameStudentCursor INTO @id, @name
	PRINT 'предыдущая строка: ' + cast(@id as char) + trim(@name);
		FETCH ABSOLUTE 20 from nameStudentCursor INTO @id, @name
	PRINT '20-я строка с начала: ' + cast(@id as char) + trim(@name);
		FETCH ABSOLUTE -20 from nameStudentCursor INTO @id, @name
	PRINT '20-я строка с конца: ' + cast(@id as char) + trim(@name);
		FETCH RELATIVE 5 from nameStudentCursor INTO @id, @name
	PRINT '5-я строка вперёд от текущей: ' + cast(@id as char) + trim(@name);
		FETCH RELATIVE -6 from nameStudentCursor INTO @id, @name
	PRINT '6-я строка назад от текущей: ' + cast(@id as char) + trim(@name);
CLOSE nameStudentCursor
DEALLOCATE nameStudentCursor
go
...
PRINT 'Количество строк: ' + CAST(@@CURSOR_ROWS AS VARCHAR(5));
	...
	DELETE SUBJECT_COPY WHERE CURRENT OF DSC_CURSOR;
		FETCH DSC_CURSOR INTO @dsc;
	UPDATE SUBJECT_COPY SET PULPIT = 'ИСиТ' WHERE CURRENT OF DSC_CURSOR;
