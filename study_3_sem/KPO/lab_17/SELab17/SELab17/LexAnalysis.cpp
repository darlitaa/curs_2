...
namespace LA
{
	void LexicalAnalyzer(const In::IN& source, LT::LexTable& lexTable, IT::IdTable& idTable)
	{
		...
		for (int i = 0; i < source.size; i++)
		{
			...
			if (source.text[i] == '\'')
			{
				...
				// Запись оставшихся символов строки
				while (source.text[i] != '\'' && source.text[i] != '\0')
				{
					// Проверка длины строкового литерала
					if (tempWordIndex >= TI_STR_MAXSIZE)
						throw ERROR_THROW_IN(91, lexemaLine, lexemaPosition); // Ошибка: строковый литерал слишком длинный

					// Считывание символов
					tempWord[tempWordIndex++] = source.text[i++];
				}

				// Проверка на то, была ли закрыта строка
				if (source.text[i] == '\0')
				{
					throw ERROR_THROW_IN(92, lexemaLine, lexemaPosition); // ошибка: строковый литерал не закрыт (не распознан)
				}
			...
			{
				// Игнорирование пробелом и табуляций
				if (source.text[i] == ' ' || source.text[i] == '\t')
					continue;

				// Запись текущего символа
				tempWord[tempWordIndex] = source.text[i];

				// В случае если оператор не распознан
				if (!AnalyzeToken(tempWord, lexemaLine, lexTable, idTable))
		...
		if (!brackets.empty()) {
			...
			{
				throw ERROR_THROW_IN(98, lexemaLine, lexemaPosition);
			}
			else if (lastBracket == '(' || lastBracket == ')')
			{
				throw ERROR_THROW_IN(99, lexemaLine, lexemaPosition);
			}
		}
	}
	...
	bool AnalyzeToken(char* token, const int str_number, LT::LexTable& lexTable, IT::IdTable& idTable)
	{
		...
		case '/':
			...
			// Проверка на то, является ли токен функцией
		case 'f':
		{
		...
		// Проверка на то, является ли токен числом
		case 'i':
		{
		...
		// Проверка на то, является ли токен стрококй
		case 's':
		{
	...
	{
		// Проверка: не может начинаться с цифры
		if (isdigit(token[0]))
			throw ERROR_THROW_IN(90, str_number, -1);

		FST::FST id(IDENTIFICATOR(token));

		if (!FST::execute(id))
			// Ошибка, идентификатор не распознан
		...
		if (strcmp(token, "main") == 0)
		{
			for (int i = 0; i < lexTable.size; i++)
				if (lexTable.GetEntry(i).lexema == LEX_MAIN) // Проверка:был ли "main" добавлен в лексическую таблицу
					throw ERROR_THROW_IN(94, str_number, -1); // Ошибка, найдено несколько функций main
		...
		// Функция
		// Проврека на слово function и стоящий перед ним тип данных
			...
			&& lexTable.GetEntry(lexTable.size - 2).lexema == LEX_DATATYPE)
		{
			if (idTable.IsId(token) != -1)
				throw ERROR_THROW_IN(95, str_number, -1); // Ошибка, ф-ция объявлена несколько раз
			...
			for (int i = lexTable.size - 1; i >= 0; i--)
			{
					...
					&& idTable.GetEntry(lexTable.GetEntry(i).idxTI).idtype == IT::IDTYPE::F)
				{
					if (idTable.IsId(token, idTable.GetEntry(lexTable.GetEntry(i).idxTI).id) != -1)
						// Ошибка, переменная объявлена несколько раз
