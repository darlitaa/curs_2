...
{
	internal class LDVFileManager
	{
		private readonly LDVLog logger;
		string drivePath;
		public LDVFileManager() 
		{
			logger = new LDVLog();
		}

		public LDVFileManager(string drive)
		{
			logger = new LDVLog();
			drivePath = drive;
		}
		...
		public void InspectDrive()
		{
			...
			string anotherPath = Path.Combine(drivePath, "LDVFiles");
			...
			string archivePath = Path.Combine(inspectPath, "LDVFiles.zip");
		...
		{
			string dirInfoCopy = Path.Combine(Path.GetDirectoryName(dirInfoPath)!, "ldvdirinfo_copy.txt");
